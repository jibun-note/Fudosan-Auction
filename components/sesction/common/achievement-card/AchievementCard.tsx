			...
			<CardContent className="px-5 pb-8 pt-4 md:px-10 md:pb-10 md:pt-5">
				<div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center sm:gap-6 md:gap-8">
					<div className="w-full max-w-56 rounded-xl border border-gray-200 bg-white px-5 py-5 text-center shadow-sm">
						<p className="text-xs font-semibold text-gray-500 md:text-base">{d.oldRentLabel}</p>
						...
						{d.oldCaption ? (
							<p className="mt-1 text-xs text-gray-500 md:text-sm">{d.oldCaption}</p>
						) : null}
					</div>
					...
					<div className="flex flex-col items-center gap-1">
						...
						<span className="text-[0.65rem] font-bold text-red-500 md:text-sm">
							{d.arrowLabel}
						</span>
					</div>
					...
					<div className="w-full max-w-56 rounded-xl border-2 border-gold/60 bg-linear-to-b from-white to-[#fff9eb] px-5 py-5 text-center shadow-md">
						<p className="text-xs font-semibold text-gold md:text-base">{d.newRentLabel}</p>
						<p className="mt-2 text-2xl font-bold tabular-nums text-navy md:text-4xl">
							{d.newRentAmount}
							<span className="text-base font-semibold md:text-xl">{d.currencyUnit}</span>
						</p>
						{d.newCaption ? (
							<p className="mt-1 text-xs font-semibold text-gold md:text-sm">{d.newCaption}</p>
						) : null}
					</div>
					...
					<div className="flex size-20 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-red-500 via-red-500 to-amber-400 text-center font-bold text-white shadow-lg ring-4 ring-red-100 md:size-24">
						<span className="flex flex-col leading-tight">
							<span className="text-xl md:text-3xl">{d.increasePercent}</span>
							<span className="text-[0.6rem] md:text-sm">{d.increaseBadgeSuffix}</span>
						</span>
					</div>
				</div>

				{d.footer ? (
					<p className="mt-8 text-center text-base font-bold leading-snug text-navy md:text-2xl md:leading-snug">
						{d.footer}
					</p>
				) : null}
			</CardContent>
